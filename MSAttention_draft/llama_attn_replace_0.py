...
def forward_mss_inline(self,q,k,v):
    ...
    patchsize=[1,2,8]
    patchscale=[torch.tensor(1.0),torch.tensor(4.0),torch.tensor(32.0)]
    local_windows=128
...
def replace_llama_attn(use_flash_attn=True, use_full=False, inference=False):
    if use_flash_attn:
        ...
        transformers.models.llama.modeling_llama.LlamaAttention.forward = forward_noflashattn0
...
class LlamaAttention_mss(LlamaAttention):
    ...
    def __init__(self, config: LlamaConfig, layer_idx=None):
        ...
        self.topk = (256,128,128)
        self.patchsize = (1,2,0)
        # self.patchscale = patchscale
        # self.patchscale1 = nn.Parameter(torch.tensor(1.0),requires_grad=True)
        # self.attend=nn.Softmax(dim=-1)
        # self.dropout=nn.Dropout(0.)
