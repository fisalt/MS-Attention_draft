...
from llama_attn_replace_o1 import forward_flashattn,forward_flashattn1,LlamaAttention_mss_ccc,LlamaAttention_mss_ccc_R2,LlamaAttention_mss_ccc_Rt,LlamaAttention_mss_ccc_t,LlamaAttention_mss_ccc_s,LlamaAttention_mss_cc1,LlamaAttention_mss_ccc_sps,LlamaAttention_mss_ccc_sps_mix
from llama_LM_replace_R import CustomTrainer3,CustomTrainer2
...
def train():
    ...
    config = transformers.AutoConfig.from_pretrained(
       f"EleutherAI/pythia-{versioins}b-deduped",
    )
    ...
    model = GPTNeoXForCausalLM.from_pretrained(
        f"EleutherAI/pythia-{versioins}B-deduped",
        revision="step3000",
        cache_dir=f"./pythia--{versioins}b-deduped/step3000",
        config=config,
        torch_dtype=torch.bfloat16,
    )
    # models = GPTNeoXForCausalLM.from_pretrained(
    ...
    #     torch_dtype=torch.bfloat16,
    # )
    from gpt_attn_replace import GPTNeoXAttention_Flash,GPTNeoXAttention_MS

    if training_args.replace_l==10:
        for i in range(len(model.gpt_neox.layers)):
            model.gpt_neox.layers[i].attention=GPTNeoXAttention_MS(config,seg=32,merge=32,top=64)
    elif training_args.replace_l==9:
        for i in range(len(model.gpt_neox.layers)):
            model.gpt_neox.layers[i].attention=GPTNeoXAttention_Flash(config)
        ...
        for i in range(len(model.model.layers)):
            ...
            model.model.layers[i].self_attn=LlamaAttention_mss_ccc_sps(config,seg=16,merge=64,top=512)
    else:
        # for i in range(len(model.model.layers)):
        #     model.model.layers[i].self_attn=LlamaAttention_mss(config)
        transformers.models.llama.modeling_llama.LlamaAttention.forward = forward_flashattn1
    ...
    if rank > 0:
        barrier()
    # dataset = load_dataset("togethercomputer/RedPajama-Data-1T-Sample", cache_dir=training_args.cache_dir, streaming=True)
    # print("data")
