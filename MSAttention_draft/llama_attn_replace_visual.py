...
def replace_llama_attn(use_flash_attn=True, use_full=False, inference=False):
    if use_flash_attn:
        ...
        transformers.models.llama.modeling_llama.LlamaAttention.forward = forward_noflashattn3
...
def Select_KV_C2(self,shape,global_tokenq,q_mask,k,v,topklist,patchsize,seg_len=2048,w=16):
    ...
    if nk>=seg_len or True:
        ...
        # offmask=torch.arange(0,n*p,p,device='cuda',dtype=torch.int)
        # offmask= repeat(offmask[:,None], 'd 1 -> d b', b = p)
        offmask=torch.arange(-p,n*p-p,p,device='cuda',dtype=torch.int)
    ...
    nk=global_tokenkm.shape[-2]
    offm=torch.arange(0,b*n*h*nk,nk,device='cuda',dtype=torch.int)
    # offm=torch.arange(0,b*nk*num_heads,nk,device='cuda',dtype=torch.int)
...
def Select_K_V_C(self,shape,global_tokenq,q_mask,k,v,topklist,patchsize,seg_len=2048,w=16):
    ...
    if nk>=seg_len or True:
        ...
        # offmask=torch.arange(0,n*p,p,device='cuda',dtype=torch.int)
        # offmask= repeat(offmask[:,None], 'd 1 -> d b', b = p)
        offmask=torch.arange(0,n*p,1,device='cuda',dtype=torch.int)
...
def Select_KV_C_mixc_visual(self,shape,global_tokenq,q_mask,kv,topklist,patchsize,seg_len=2048,w=16):
    # b,h,n,s,d=shape
    # num_heads=h
    backward=1
...
# def forward_mss_local_inline_for1(self,q,k,v,topklist=[128,64,64],patchsize=[1,2,8]):
    ...
    k0,v0,mask0,select0=Select_KV_C_mixc_visual(self,q.shape,global_tokenq,q_mask,kv=torch.cat((k,v),dim=-1),topklist=topklist[0],patchsize=patchsize[0],seg_len=seg_len)
    Kt.append(k0)
    Vt.append(v0)
    Mask.append(mask0)
    Select.append(select0)
    indx=topklist[0]*patchsize[0]
    ...
    for i in range(1,len(topklist)):
        ...
        ki,vi,maski,selecti=Select_KV_C_mixc_visual(self,q.shape,global_tokenq,q_mask,kv=torch.cat((k,v),dim=-1),topklist=topklist[i],patchsize=patchsize[i],seg_len=seg_len)
        Kt.append(ki)
        Vt.append(vi)
        Mask.append(maski)
    ...
    v=torch.cat(Vt, dim=-2)
    kv_mask=torch.cat(Mask, dim=-1).bool()
    Select=torch.cat(Select, dim=-1)
    Select=repeat(Select[...,None,:], 'b h n s 1 t -> b h (n s w) t',w=w)
    # patchscale=torch.cat(self.patchscale, dim=-1)
    ...
    # print("v",v.shape)
    attn=nn.functional.softmax(dots, dim=-1)
    attn1=repeat(attn, 'b h n s w t -> b h (n s w) t')
    visualize_attention_map(Select[0,0].int().cpu().numpy(),attn1[0,0].float().cpu().numpy(),"atten_map1")
    quit()
    out=torch.matmul(attn,v)
...
def visualize_attention_map(selected_cols, attention_scores, output_file, m=256):
    """
    ...
    # attention_map = np.full((n, n), 0.0)
    attention_map = np.full((n+1, n+1), 0.0)
    ...
    attention_map[rows, selected_cols] = attention_scores.reshape(-1, selected_cols.shape[1])
    attention_map=attention_map[:m,:m]
