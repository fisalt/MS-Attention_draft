...
class LlamaAttention_mss(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx=None):
        ...
        patchscale.append(1.0*32*torch.ones((128)))
        self.patchscale=nn.Parameter(torch.cat(patchscale),requires_grad=True)
        # self.patchscale=nn.ParameterList()
        # # self.patchscale=[]
        # # self.patchscale.append(nn.Parameter(torch.tensor(1.0),requires_grad=True))
        # # self.patchscale.append(nn.Parameter(torch.tensor(4.0,requires_grad=True),requires_grad=True))
        ...
        # self.patchscale.append(nn.Parameter(16.0*torch.ones((self.topk[2])),requires_grad=True))
    ...
    def forward(
        ...
        #select1
        attn_output,attn_weights=forward_mss_local_inline_own(self,query_states,key_states,value_states)
...
def forward_mss_local_inline_for3(self,q,k,v,topklist=[128,64,64],patchsize=[1,4,16]):
    ...
    # print("LlamaAttention_mss")
    autopad=True
    # print(self.patchscale.weight[:5])
    # print(self.patchscale.weight[256:260])
    # print(self.patchscale.weight[320:325])
