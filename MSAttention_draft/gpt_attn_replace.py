...
from llama_attn_replace_o1 import forward_mss_local_inline_for_cccp_dttm
...
class GPTNeoXAttention_Flash(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.num_attention_heads = config.num_attention_heads
    ...
    def forward(
        ...
        attn_output=flash_attn_func(query.transpose(1,2),key.transpose(1,2),value.transpose(1,2),causal=True).transpose(1,2)
        attn_weights=None
...
class GPTNeoXAttention_MS(nn.Module):
    def __init__(self, config, seg=16, top=256, merge=128):
        super().__init__()
        self.seg = seg
        self.top = top
        self.merge = merge
    ...
    def forward(
        ...
        # Compute attention
        # attn_output, attn_weights = self._attn(query, key, value, attention_mask, head_mask)
        attn_output,attn_weights=forward_mss_local_inline_for_cccp_dttm(self,query,key,value,topklist=[self.top],patchsize=[self.seg],w=self.seg,mergep=self.merge,flash_use=True)
        # attn_output=flash_attn_func(query.transpose(1,2),key.transpose(1,2),value.transpose(1,2),causal=True).transpose(1,2)
        # attn_weights=None
