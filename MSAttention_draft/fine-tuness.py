...
from llama_attn_replace_o1 import LlamaAttention_mss_ccc,LlamaAttention_mss_ccc_R2,LlamaAttention_mss_ccc_Rt,LlamaAttention_mss_ccc_t,LlamaAttention_mss_ccc_s,LlamaAttention_mss_cc1,LlamaAttention_mss_ccc_sps,LlamaAttention_mss_ccc_sps_mix
...
def train():
    ...
    if training_args.replace_l==2:
        ...
        for i in range(len(model.model.layers)):
            model.model.layers[i].self_attn=LlamaAttention_mss_ccc_s(config)
    else:
        for i in range(len(model.model.layers)):
    ...
    if rank > 0:
        barrier()
    dataset = load_dataset("togethercomputer/RedPajama-Data-1T-Sample", cache_dir=training_args.cache_dir, load_from_cache_file=True)
    ...
    #     if "embed" in n or "norm" in n  or "k_proj" in n:
    #         p.requires_grad=True
        # else:
        #     p.requires_grad=False

        # if "embed" in n and "norm" in n and "o_proj" in n and "k_proj" in n:
        #     p.requires_grad=False
