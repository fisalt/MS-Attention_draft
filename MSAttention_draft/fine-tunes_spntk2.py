...
from llama_attn_replace_o1 import forward_flashattn,forward_LBe,forward_flashattn1,LlamaAttention_mss_ccc,LlamaAttention_mss_ccc_R2,LlamaAttention_mss_ccc_Rt,LlamaAttention_mss_ccc_t,LlamaAttention_mss_ccc_s,LlamaAttention_mss_cc1,LlamaAttention_mss_ccc_sps,LlamaAttention_mss_ccc_sps_mix
...
def train():
    ...
    # dataset = dataset.map(partial(tokenize_fn,tokenizer),batched=True, num_proc=16, remove_columns=["text", "meta"], load_from_cache_file=True)
    from datasets import concatenate_datasets

    def process_batch(batch):
        return batch.map(partial(tokenize_fn, tokenizer), batched=True, num_proc=16, remove_columns=["text", "meta"], load_from_cache_file=True)

    batch_size = 100000  # 根据需要调整批次大小
    batched_datasets = [dataset.shard(num_shards=len(dataset) // batch_size, index=i) for i in range(len(dataset) // batch_size)]
    processed_datasets = [process_batch(batch) for batch in batched_datasets]
