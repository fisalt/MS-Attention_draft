...
from llama_attn_replace import replace_llama_attn, LlamaAttention_mss
...
from llama_attn_replace import replace_llama_attn, LlamaAttention_mss,LlamaAttention_mss_mix,LlamaAttention_mss_c,LlamaAttention_mss_cc,LlamaAttention_mss_ccc,LlamaAttention_mss_ccc_nope,LlamaAttention_mss_ccc_fpe
...
class Pg19Dataset(Dataset):
    ...
    def __getitem__(self, index) -> typing.Tuple[str, torch.Tensor]:
...
class EvalMetric(ABC):
    @abstractmethod
    def add(self, logits: torch.FloatTensor, labels: torch.LongTensor, model_output: object) -> typing.Tuple[str, object]:
        pass

    @abstractmethod
    def compute(self) -> typing.Tuple[str, object]:
        pass
...
class DistributedEvaluator:
    ...
    def evaluate(self, dataset: Dataset, metric: EvalMetric) -> typing.Tuple[str, object]:
...
def run_eval(args: EvalArguments):
    ...
    dataset = Pg19Dataset(args.data_path, seq_length=args.seq_len, sliding_window=args.sliding_window)
    if args.flash_attn:
        replace_llama_attn(use_flash_attn=True, use_full=True, ss=args.replace_l)
    replace_llama_attn(use_flash_attn=False, use_full=False, ss=args.replace_l)
    ...
    models = transformers.AutoModelForCausalLM.from_pretrained(
        args.base_model,
        config=config,
        cache_dir=args.cache_dir,
        torch_dtype=torch_dtype)
    ...
    if args.replace_l==2:
        ...
        for i in range(len(model.model.layers)):
            model.model.layers[i].self_attn=LlamaAttention_mss_ccc_fpe(config)
            # LlamaAttention_mss_ccc_nope
    else:
        for i in range(len(model.model.layers)):
    ...
    if args.peft_model:
        ...
        # trainable_params = os.path.join(args.peft_model, "training_args.bin")
